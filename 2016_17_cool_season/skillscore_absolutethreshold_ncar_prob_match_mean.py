...
links = ["/uufs/chpc.utah.edu/common/home/steenburgh-group5/tom/snotel_data/2016_17_cool_season/snotel_precip_2016_2017_qc.csv",
         "/uufs/chpc.utah.edu/common/home/steenburgh-group5/tom/snotel_data/2016_17_cool_season/ncarens8_precip_12Zto12Z_interp.txt",
...
#        if ncar_data[x,0] == inhouse_data[y,0] == 651:
#            for percent in range(0,100,5):
#                print percent
#                p_array = inhouse_data[y,3:185]
#                p_array_f = ncar_data[x,3:185]
#                 
            ...
            if w == 0:
                    if ((regions[0,0] <= ncar_data[x,1] <= regions[0,1] and regions[0,2] <= ncar_data[x,2] <= regions[0,3]) or ###Sierra Nevada
                        ...
                        if ncar_data[x,0] == inhouse_data[y,0]:
                            ...
                            ncar_mean_pac = np.append(ncar_mean_pac, ncar_mean_data[x,3:185])
                            ncar_prob_match_mean_pac = np.append(ncar_prob_match_mean_pac, ncar_prob_match_mean_data[x,3:185])
            ...
            if w == 1:
                    if ((regions[2,0] <= ncar_data[x,1] <= regions[2,1] and regions[2,2] <= ncar_data[x,2] <= regions[2,3]) or ## CO Rockies
                        ...
                        if ncar_data[x,0] == inhouse_data[y,0]:
                            ...
                            ncar_mean_int = np.append(ncar_mean_int, ncar_mean_data[x,3:185])
                            ncar_prob_match_mean_int = np.append(ncar_prob_match_mean_int, ncar_prob_match_mean_data[x,3:185])
...
#                                                      
                            ...
                            for z in range(3,185):
                                ...
                                if all(data[1:,x,z] < 1000) and  inhouse_data[y,z] < 1000:
                                    ...
                                    if ncar_data[x,z] >= abs_thres[s] and inhouse_data[y,z] >= abs_thres[s]:
                                        hit[s,u] = hit[s,u] + 1
...
#                            
                            ...
                            for z in range(3,185):
                                ...
                                if all(data[1:,x,z] < 1000) and  inhouse_data[y,z] < 1000:
                                    ...
                                    if gfs_data[x,z] >= abs_thres[s] and inhouse_data[y,z] >= abs_thres[s]:
                                        hit[s,u] = hit[s,u] + 1
...
#                            
                            ...
                            for z in range(3,185):
                                ...
                                if all(data[1:,x,z] < 1000) and  inhouse_data[y,z] < 1000:
                                    ...
                                    if hrrr_data[x,z] >= abs_thres[s] and inhouse_data[y,z] >= abs_thres[s]:
                                        hit[s,u] = hit[s,u] + 1
...
#                            percentile_f = np.percentile(p_array_f,percent)
                            ...
                            for z in range(3,185):
                                ...
                                if all(data[1:,x,z] < 1000) and  inhouse_data[y,z] < 1000:
                                    ...
                                    if nam_data[x,z] >= abs_thres[s] and inhouse_data[y,z] >= abs_thres[s]:
                                        hit[s,u] = hit[s,u] + 1
...
#                            
                            ...
                            for z in range(3,185):
                                ...
                                if all(data[1:,x,z] < 1000) and  inhouse_data[y,z] < 1000:
                                    ...
                                    if sref_arw_data[x,z] >= abs_thres[s] and inhouse_data[y,z] >= abs_thres[s]:
                                        hit[s,u] = hit[s,u] + 1
...
#                            
                            ...
                            for z in range(3,185):
                                ...
                                if all(data[1:,x,z] < 1000) and  inhouse_data[y,z] < 1000:
                                    ...
                                    if sref_nmb_data[x,z] >= abs_thres[s] and inhouse_data[y,z] >= abs_thres[s]:
                                        hit[s,u] = hit[s,u] + 1
...
###############################################################################
    ...
    for a in range(1,end+1):
        percent = a * 5
        s = a-1
        for x in range(len(ncar_mean_data[:,0])):
            for y in range(len(inhouse_data[:,0])):
                                                ################### PACIFIC ###################            
                if w == 0:
                    if ((regions[0,0] <= ncar_mean_data[x,1] <= regions[0,1] and regions[0,2] <= ncar_mean_data[x,2] <= regions[0,3]) or ###Sierra Nevada
                            
                        (regions[1,0] <= ncar_mean_data[x,1] <= regions[1,1] and regions[1,2] <= ncar_mean_data[x,2] <= regions[1,3]) and  ##Far NW minus bottom right(>42.97, <-121.69)
                        (ncar_mean_data[x,1] >= regions[1,4] or ncar_mean_data[x,2] <= regions[1,5])):
                        if ncar_mean_data[x,0] == inhouse_data[y,0]:
                
              
#                            #p_array is all precip days for one station.  Created to determine percentiles for each station
#                            p_array = inhouse_pac
#                            p_array_f = ncar_mean_pac
#                            
#                            ## NAM is only mondel with bad data
#                            p_array = np.delete(p_array, np.where(nam_pac > 1000))
#                            p_array_f = np.delete(p_array_f, np.where(nam_pac > 1000))
...
#                            
                            ...
                            for z in range(3,185):
                                ...
                                if all(data[1:,x,z] < 1000) and  inhouse_data[y,z] < 1000:
                                    ...
                                    if ncar_mean_data[x,z] >= abs_thres[s] and inhouse_data[y,z] >= abs_thres[s]:
                                        hit[s,u] = hit[s,u] + 1
                                    ...
                                    ################  INTERMOUNTAIN #################                                        
                if w == 1:
                    if ((regions[2,0] <= ncar_mean_data[x,1] <= regions[2,1] and regions[2,2] <= ncar_mean_data[x,2] <= regions[2,3]) or ## CO Rockies
    
                        (regions[3,0] <= ncar_mean_data[x,1] <= regions[3,1] and regions[3,2] <= ncar_mean_data[x,2] <= regions[3,3]) and  ### Intermounaint mimus bottom right and top left (> 39.32, < -109.54, <43.6, > -111.38)
                        (ncar_mean_data[x,1] >= regions[3,4] or ncar_mean_data[x,2] <= regions[3,5]) and 
                        (ncar_mean_data[x,1] <= regions[3,6] or ncar_mean_data[x,2] >= regions[3,7]) or
                        
                        (regions[4,0] <= ncar_mean_data[x,1] <= regions[4,1] and regions[4,2] <= ncar_mean_data[x,2] <= regions[4,3]) and  ### Intermountain NW minus bottom left and bottom right ( > 45.28, > -115.22, > 44.49, < -110.84)
                        (ncar_mean_data[x,1] >= regions[4,4] or ncar_mean_data[x,2] >= regions[4,5]) and 
                        (ncar_mean_data[x,1] >= regions[4,6] or ncar_mean_data[x,2] <= regions[4,7]) or
                        
                        (regions[5,0] <= ncar_mean_data[x,1] <= regions[5,1] and regions[5,2] <= ncar_mean_data[x,2] <= regions[5,3]) and  ### SW ID minus top right (< 44.46, <-114.5)
                        (ncar_mean_data[x,1] <= regions[5,4] or ncar_mean_data[x,2] <= regions[5,5])):  
                        if ncar_mean_data[x,0] == inhouse_data[y,0]:
                                                    #p_array is all precip days for one station.  Created to determine percentiles for each station
#                            p_array = inhouse_int
#                            p_array_f = ncar_mean_int
#                            
#                            ## NAM is only mondel with bad data
#                            p_array = np.delete(p_array, np.where(nam_int > 1000))
#                            p_array_f = np.delete(p_array_f, np.where(nam_int > 1000))
...
###############################################################################
    ...
    for a in range(1,end+1):
        percent = a * 5
        s = a-1
        for x in range(len(ncar_prob_match_mean_data[:,0])):
            for y in range(len(inhouse_data[:,0])):
                                                ################### PACIFIC ###################            
                if w == 0:
                    if ((regions[0,0] <= ncar_prob_match_mean_data[x,1] <= regions[0,1] and regions[0,2] <= ncar_prob_match_mean_data[x,2] <= regions[0,3]) or ###Sierra Nevada
                            
                        (regions[1,0] <= ncar_prob_match_mean_data[x,1] <= regions[1,1] and regions[1,2] <= ncar_prob_match_mean_data[x,2] <= regions[1,3]) and  ##Far NW minus bottom right(>42.97, <-121.69)
                        (ncar_prob_match_mean_data[x,1] >= regions[1,4] or ncar_prob_match_mean_data[x,2] <= regions[1,5])):
                        if ncar_prob_match_mean_data[x,0] == inhouse_data[y,0]:
                
              
#                            #p_array is all precip days for one station.  Created to determine percentiles for each station
#                            p_array = inhouse_pac
#                            p_array_f = ncar_prob_match_mean_pac
#                            
#                            ## NAM is only mondel with bad data
#                            p_array = np.delete(p_array, np.where(nam_pac > 1000))
#                            p_array_f = np.delete(p_array_f, np.where(nam_pac > 1000))
...
#                            
                            ...
                            for z in range(3,185):
                                ...
                                if all(data[1:,x,z] < 1000) and  inhouse_data[y,z] < 1000:
                                    ...
                                    if ncar_prob_match_mean_data[x,z] >= abs_thres[s] and inhouse_data[y,z] >= abs_thres[s]:
                                        hit[s,u] = hit[s,u] + 1
                                    ...
                                    ################  INTERMOUNTAIN #################                                        
                if w == 1:
                    if ((regions[2,0] <= ncar_prob_match_mean_data[x,1] <= regions[2,1] and regions[2,2] <= ncar_prob_match_mean_data[x,2] <= regions[2,3]) or ## CO Rockies
    
                        (regions[3,0] <= ncar_prob_match_mean_data[x,1] <= regions[3,1] and regions[3,2] <= ncar_prob_match_mean_data[x,2] <= regions[3,3]) and  ### Intermounaint mimus bottom right and top left (> 39.32, < -109.54, <43.6, > -111.38)
                        (ncar_prob_match_mean_data[x,1] >= regions[3,4] or ncar_prob_match_mean_data[x,2] <= regions[3,5]) and 
                        (ncar_prob_match_mean_data[x,1] <= regions[3,6] or ncar_prob_match_mean_data[x,2] >= regions[3,7]) or
                        
                        (regions[4,0] <= ncar_prob_match_mean_data[x,1] <= regions[4,1] and regions[4,2] <= ncar_prob_match_mean_data[x,2] <= regions[4,3]) and  ### Intermountain NW minus bottom left and bottom right ( > 45.28, > -115.22, > 44.49, < -110.84)
                        (ncar_prob_match_mean_data[x,1] >= regions[4,4] or ncar_prob_match_mean_data[x,2] >= regions[4,5]) and 
                        (ncar_prob_match_mean_data[x,1] >= regions[4,6] or ncar_prob_match_mean_data[x,2] <= regions[4,7]) or
                        
                        (regions[5,0] <= ncar_prob_match_mean_data[x,1] <= regions[5,1] and regions[5,2] <= ncar_prob_match_mean_data[x,2] <= regions[5,3]) and  ### SW ID minus top right (< 44.46, <-114.5)
                        (ncar_prob_match_mean_data[x,1] <= regions[5,4] or ncar_prob_match_mean_data[x,2] <= regions[5,5])):  
                        if ncar_prob_match_mean_data[x,0] == inhouse_data[y,0]:
                                                    #p_array is all precip days for one station.  Created to determine percentiles for each station
#                            p_array = inhouse_int
#                            p_array_f = ncar_prob_match_mean_int
#                            
#                            ## NAM is only mondel with bad data
#                            p_array = np.delete(p_array, np.where(nam_int > 1000))
#                            p_array_f = np.delete(p_array_f, np.where(nam_int > 1000))
...
x = np.arange(2.54,49,2.54)
xtick = np.arange(7.64,49,5.08)
linecolor = ['blue', 'green', 'red', 'c', 'gold', 'magenta', 'lime', 'sienna']
...
plt.gca().set_color_cycle(linecolor)
ax2.plot(x,hitrate[:,0:8],linewidth = 2,marker = "o", markeredgecolor = 'none')
plt.grid(True)
blue = mlines.Line2D([], [], color='blue',
                           label='NCAR ENS CTL', linewidth = 2,marker = "o", markeredgecolor = 'none')
green = mlines.Line2D([], [], color='green',
...
lime = mlines.Line2D([], [], color='lime',
                           label='NCAR MEAN', linewidth = 2,marker = "o", markeredgecolor = 'none')
sienna = mlines.Line2D([], [], color='sienna',
                           label='NCAR PROB MATCH MEAN', linewidth = 2,marker = "o", markeredgecolor = 'none')
props = dict(boxstyle='square', facecolor='white', alpha=1)
...
plt.xlim([7.62,48.27])
plt.xticks(xtick)
ax2.set_xticklabels(np.around(xtick, decimals = 1), fontsize = 12)
#ax2.tick_params(axis='y', labelsize=14)
locx = 30
locy = 30
font = 25
fig1.text(0.129, 0.70, '(a)', fontsize = font)
...
plt.gca().set_color_cycle(linecolor)
ax2.plot(x,hitrate[:,10:18],linewidth = 2,marker = "o", markeredgecolor = 'none')
plt.grid(True)
...
#ax2.tick_params(axis='y', labelsize=14)
fig1.text(0.552, 0.70, '(b)', fontsize = font)
...
plt.gca().set_color_cycle(linecolor)
ax3.plot(x,faratio[:,0:8],linewidth = 2,marker = "o", markeredgecolor = 'none')
plt.grid(True)
...
fig1.text(0.129, 0.453, '(c)', fontsize = font)



ax3 = fig1.add_subplot(324, sharex = ax2)
plt.gca().set_color_cycle(linecolor)
ax3.plot(x,faratio[:,10:18],linewidth = 2,marker = "o", markeredgecolor = 'none')
plt.grid(True)
#plt.xlim([75,95.1])
plt.ylim([0,1])
plt.xlim([7.62,48.27])
...
#ax3.set_xticklabels(np.arange(5,96,5), fontsize = 14)
#ax3.tick_params(axis='y', labelsize=14)
fig1.text(0.552, 0.453, '(d)', fontsize = font)


ax4 = fig1.add_subplot(325, sharex = ax2)
plt.gca().set_color_cycle(linecolor)
ax4.plot(x,ets[:,0:8],linewidth = 2,marker = "o", markeredgecolor = 'none')
plt.grid(True)
plt.ylabel('ETS', fontsize = 20)
plt.xlabel('Event Threshold (mm)', fontsize = 20)
#plt.xlim([75,95.1])
plt.ylim([0,0.45])
plt.xlim([7.62,48.27])
plt.xticks(xtick)
...
fig1.text(0.129, 0.2043, '(e)', fontsize = font)



ax4 = fig1.add_subplot(326, sharex = ax2)
plt.gca().set_color_cycle(linecolor)
ax4.plot(x,ets[:,10:18],linewidth = 2,marker = "o", markeredgecolor = 'none')
plt.grid(True)
plt.xlabel('Event Threshold (mm)', fontsize = 20)
#plt.xlim([75,95.1])
plt.ylim([0,0.45])
...
#ax4.set_xticklabels(np.arange(5,96,5), fontsize = 14)
#ax4.tick_params(axis='y', labelsize=14)
fig1.text(0.552, 0.2043, '(f)', fontsize = font)




plt.legend(handles=[ blue, red, cyan,green, gold, magenta, lime, sienna], loc='upper center', bbox_to_anchor=(-0.20, -0.21), 
           ncol=2, fontsize = 18)
plt.savefig("../../../public_html/ms_thesis_plots/categorical_scores_absoluteforecasts_interp_regional_2016_17_ncar_prob_match_mean_1.pdf")
plt.close(fig1)
